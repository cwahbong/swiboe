...
pub struct Client {
    ...
    rpc_caller_commands: rpc_caller::CommandSender,

    // The thread dealing with all the logic in the client.
    rpc_caller_thread: Option<thread::JoinHandle<()>>,
    rpc_callee_thread: Option<thread::JoinHandle<()>>,
...
impl Client {
    ...
    fn common_connect<Reader: io::Read + Send + 'static, Writer: io::Write + Send + 'static>(reader_stream: Reader, writer_stream: Writer, shutdown_func: Box<Fn() -> ()>) -> Self {
        let (recv_tx, recv_rx) = mpsc::channel::<ipc::Message>();
        let (send_tx, send_rx) = mpsc::channel::<ipc::Message>();
        let (commands_tx, commands_rx) = mpsc::channel::<rpc_caller::Command>();

        let read_recv_tx = recv_tx.clone();
        let read_thread = thread::spawn(move || {
            let mut reader = ipc::Reader::new(reader_stream);
            while let Ok(message) = reader.read_message() {
                if read_recv_tx.send(message).is_err() {
                    break;
                }
            };
        });
        ...
        Client {
            rpc_caller_commands: commands_tx.clone(),
            rpc_caller_thread: Some(rpc_caller::spawn(commands_rx, send_tx)),
            rpc_callee_thread: Some(rpc_callee::spawn(recv_rx, commands_tx.clone())),
    ...
    pub fn new_rpc(&mut self, name: &str, rpc: Box<rpc::server::Rpc>) -> Result<()> {
        ...
        self.rpc_caller_commands.send(rpc_caller::Command::NewRpc(name.into(), rpc)).expect("NewRpc");
        Ok(())
    }

    pub fn clone(&self) -> Result<ThinClient> {
        Ok(ThinClient {
            rpc_caller_commands: Mutex::new(self.rpc_caller_commands.clone()),
        })
    }
}
...
impl RpcCaller for Client {
    fn call<T: serde::Serialize>(&mut self, function: &str, args: &T) -> Result<rpc::client::Context> {
        rpc::client::Context::new(self.rpc_caller_commands.clone(), function, args)
    }
}
...
impl Drop for Client {
    fn drop(&mut self) {
        let _ = self.rpc_caller_commands.send(rpc_caller::Command::Quit);
        if let Some(thread) = self.rpc_caller_thread.take() {
            thread.join().expect("Joining rpc_caller_thread failed.");
        }
        if let Some(thread) = self.rpc_callee_thread.take() {
            thread.join().expect("Joining rpc_callee_thread failed.");
        }
...
pub struct ThinClient {
    rpc_caller_commands: Mutex<rpc_caller::CommandSender>,
}

impl ThinClient {
    pub fn clone(&self) -> Self {
        let commands = {
            let commands = self.rpc_caller_commands.lock().unwrap();
            commands.clone()
        };
        ThinClient {
            rpc_caller_commands: Mutex::new(commands),
        }
    }
}
